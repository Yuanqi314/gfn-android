...
grep -Fq 'StreamCapabilityProfiles.V610_ANDROID_WEBRTC.rejectionReason(config)' stream-webrtc/src/main/java/dev/gfn/webrtc/GfnWebRtcEngine.kt || { echo 'ERROR: WebRTC engine capability validation drifted from v6.1 resolver' >&2; exit 1; }
...
# v6.0.4 HEVC Main / SDR8 production closeout plus v6.1.0 Main10/SDR10 negotiation regression.
sh ./verify-hevc.sh
